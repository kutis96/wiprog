...
void ConfigStore::load() {
  ...
  while (configDir.next()) {
    if(configDir.fileSize() > 0 && configDir.isFile()) { //no recursing, ignore directories.
...
bool ConfigStore::contains(char* key) {
  return configData.count(key) > 0;
}

void ConfigStore::unset(char *key) {
...
void ConfigStore::setString(char* key, String value) {
  configData[key] = value;
  save();
}

String ConfigStore::getString(char* key, String defaultValue) {
  if(!contains(key)) {
